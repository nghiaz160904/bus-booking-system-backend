...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // --- 1. CLEANUP OLD DATA ---
        // We force a cleanup so your recent code changes (gridRow/gridCol) take effect.
        log.info("Cleaning up old database data...");
        clearDatabase();
    ...
    /**
     * Wipes all data to ensure a clean state.
     * Deletes in specific order to avoid Foreign Key Constraint violations.
     */
    private void clearDatabase() {
        // 1. Child tables first (Dependencies)
        seatStatusRepository.deleteAll(); // Refers to Trip, Seat
        tripRepository.deleteAll();       // Refers to Bus, Route, Operator
        
        seatRepository.deleteAll();       // Refers to Bus, SeatType
        
        // 2. Middle tables
        routeRepository.deleteAll();      // Refers to Operator
        busRepository.deleteAll();        // Refers to Operator
        
        // 3. Parent tables last
        seatTypeRepository.deleteAll();   // Refers to Operator
        operatorRepository.deleteAll();   // Root parent
